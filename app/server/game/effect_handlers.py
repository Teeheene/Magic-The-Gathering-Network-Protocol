...
def resolve_lightning_bolt(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 3, item.source, state)

def resolve_shock(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 2, item.source, state)

def resolve_lava_spike(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 3, item.source, state)

def resolve_flame_slash(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 4, item.source, state)

def resolve_searing_spear(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 3, item.source, state)

def resolve_incinerate(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 3, item.source, state)

def resolve_skullcrack(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
...
def resolve_unsummon(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.return_to_hand(target, state)

def resolve_ponder(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    return FX.draw_cards(item.controller, 1, state)

def resolve_giant_growth(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
...
def resolve_swords_to_plowshares(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
...
def resolve_path_to_exile(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.exile_permanent(target, state)

def resolve_healing_salve(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
...
def resolve_terror(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.destroy_permanent(target, state)

def resolve_doom_blade(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.destroy_permanent(target, state)

def resolve_raise_dead(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.return_from_graveyard(target, item.controller, state)

def resolve_mind_rot(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    hand = state.hands.get(target, [])
    to_discard = hand[:2]
    return FX.discard_cards(target, to_discard, state)

def resolve_gray_merchant(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    return []
...
def resolve_rod_of_ruin(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 1, item.source, state)

def resolve_prodigal_sorcerer(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
    target = item.targets[0] if item.targets else ""
    return FX.deal_damage(target, 1, item.source, state)

def resolve_merfolk_looter(item: Any, state: GameState, game_stack: Optional[Any] = None) -> List[Dict[str, Any]]:
